...
- Installable package at `skills/human-responsibility-mapping/`, verified end to end with `npx skills add p-toni/human-responsibility-mapping`; the installed copy retains all referenced templates, examples, schemas, and scripts.
...
- `agents/openai.yaml`: OpenAI Codex UI metadata and explicit `$human-responsibility-mapping` invocation; package validation and fresh Codex forward tests complement the Anthropic-specific trigger artifacts.
